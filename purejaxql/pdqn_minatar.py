...
def make_train(config):
    ...
    def train(rng):
        ...
        def create_model(rng):
            ...
            # Initialize parameters by passing a dummy input
            init_x = jnp.zeros((1, *env.observation_space(env_params).shape))
        ...
        def _update_step(runner_state, unused):
            ...
            def _step_env(carry, _):
                ...
                # getting the a' for each env
                new_action = jax.vmap(eps_greedy_exploration)(_rngs, q_vals, eps)

                # s'ᵢ, rᵢ ← P_S(s_i, a_i), r_i ~ P_R(s_i, a_i)
            ...
            # take the max q value for each env
            last_q = jnp.max(last_q, axis=-1)

            def _get_target(lambda_returns_and_next_q, transition):
                """ Calculate the lambda returns and the next q value for the current transition """
            ...
            def _learn_epoch(carry, _):
                ...
                def _learn_phase(carry, minibatch_and_target):
                    ...
                    def _loss_fn(params):
                        ...
                        # Loss is mean squared error between the Q-values of the chosen actions and the target
